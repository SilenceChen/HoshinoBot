...
class CECounter:
    ...
    def _get_gecha_num(self, gid, uid):
        with self._connect() as conn:
            r = conn.execute(
                f"SELECT XNUM,DNUM FROM EQUIPGECHA WHERE GID={gid} AND UID={uid}").fetchall()
            return r[0] if r else [0,0]
    
    def _add_gecha_num(self, gid, uid, xnum, dnum):
        with self._connect() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO EQUIPGECHA (GID, UID, XNUM, DNUM) VALUES (?, ?, ?, ?)",
                (gid, uid, xnum, dnum),
            )
